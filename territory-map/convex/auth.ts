...
export const { auth, signIn, signOut, store, isAuthenticated } = convexAuth({
  providers: [
    Password({
      // Email verification TEMPORARILY disabled: the verify/reset senders point
      // at Viktor's decommissioned email API, which would strand every new
      // signup at the code screen. Re-enable with a Resend-backed sender once
      // the Resend account/API key exists. (ViktorSpacesEmail kept for reference.)
