...
export const getMatches = query({
  ...
  handler: async (ctx) => {
    ...
    const prospectCapital = capitalToNumber(prospect.liquidCapital);
    ...
    // Include the CONTACT state too — users update "my state" there and expect
    // matches to follow (primary territory remains the strongest signal).
    const stateEntries = [prospect.primaryState, prospect.secondaryState, (prospect as any).state]
      .map((s) => ({ code: toStateCode(s), label: s as string }))
      .filter((e): e is { code: string; label: string } => !!e.code)
      .filter((e, i, arr) => arr.findIndex((x) => x.code === e.code) === i);
    const prospectStateCodes = stateEntries.map((e) => e.code);
    ...
    for (const brand of brands) {
      const fp = fpMap.get(brand._id.toString());
      ...
      if (prospect.preferredCategories?.length && brand.category) {
        ...
        const matchCat = prospect.preferredCategories.some(
          (c) => c === brandCatLower || brand.category?.toLowerCase().includes(c.replace(/_/g, " "))
        );
        if (matchCat) {
          categoryScore = 8;
      ...
      if (prospectOwnerModels.length > 0 && fp?.ownerTypes?.length) {
        const overlap = prospectOwnerModels.filter((m: string) => fp.ownerTypes!.includes(m));
        if (overlap.length > 0) {
          ownerScore += 5;
